...
def _verdict(verdicts: list[AuditVerdict], section: str, check: str,
             result: str, detail: str = "", diagnostics: list[str] | None = None) -> None:
...
def audit_bundle(bundle: Path,
                 ...
                 fit_model: str = "notch") -> AuditReport:
    """Generate a complete audit report for a validated bundle."""
    manifest = load_manifest(bundle / "manifest.json")
    verdicts: list[AuditVerdict] = []

    # ── Provenance checks ──────────────────────────────────────────────
    _verdict(verdicts, "provenance", "manifest_loaded",
             "PASS", f"Schema {manifest.schema_version}, exporter {manifest.exporter_version}")

    if manifest.project_file_sha256:
        _verdict(verdicts, "provenance", "project_hash",
                 "PASS", f"SHA-256: {manifest.project_file_sha256[:16]}...")
    else:
        _verdict(verdicts, "provenance", "project_hash",
                 "FAIL", "Source project hash is missing")

    _verdict(verdicts, "provenance", "aedt_version",
             "PASS" if manifest.aedt_version else "WARNING",
             f"AEDT {manifest.aedt_version} / PyAEDT {manifest.pyaedt_version}")

    _verdict(verdicts, "provenance", "solution_kind",
             "PASS", f"{manifest.solution_kind.value} — {manifest.setup_name}")

    # ── Evidence completeness ──────────────────────────────────────────
    _verdict(verdicts, "evidence", "touchstone_present",
             "PASS" if manifest.touchstone else "NOT_EVALUATED",
             f"{manifest.touchstone.number_of_ports} ports" if manifest.touchstone else "")

    _verdict(verdicts, "evidence", "eigenmodes_present",
             "PASS" if manifest.eigenmode else "NOT_EVALUATED",
             f"{manifest.eigenmode.mode_count} modes" if manifest.eigenmode else "")

    _verdict(verdicts, "evidence", "fields_present",
             "PASS" if manifest.fields else ("FAIL" if manifest.evidence_profile == "strict" else "NOT_EVALUATED"),
             f"{len(manifest.fields)} field records")

    conv_present = any(f.role == "convergence" for f in manifest.files)
    _verdict(verdicts, "evidence", "convergence_present",
    ...
    _verdict(verdicts, "evidence", "mesh_stats_present",
    ...
    _verdict(verdicts, "validation", "export_status",
             "PASS" if manifest.bundle_status.value in ("complete", "complete_with_warnings") else "FAIL",
             f"Bundle status: {manifest.bundle_status.value}")
    _verdict(verdicts, "validation", "export_errors",
             "FAIL" if diag_errors else "PASS",
             f"{len(diag_errors)} errors" if diag_errors else "No errors")
    _verdict(verdicts, "validation", "export_warnings",
    ...
    if conv_present:
        try:
            ...
            _verdict(verdicts, "convergence", "is_converged",
            ...
            if convergence.false_convergence_risk not in ("low", "not_evaluated"):
                _verdict(verdicts, "convergence", "false_convergence",
            ...
            if convergence.oscillation_detected:
                _verdict(verdicts, "convergence", "oscillation",
                         "WARNING", "Oscillation detected in convergence sequence")
            if convergence.insufficient_passes:
                _verdict(verdicts, "convergence", "insufficient_passes",
                         "FAIL", "Insufficient adaptive passes for convergence assessment")
        except Exception as exc:
            _verdict(verdicts, "convergence", "audit_failed",
                     "FAIL", str(exc))
    ...
    if manifest.touchstone:
        for response in ["S21"]:
            try:
                ...
                fit_result.q_coupling_absolute = float(abs(fit_result.q_coupling_absolute)) if fit_result.q_coupling_absolute else None
                fit_results[response] = fit_result
                _verdict(verdicts, "fitting", f"{response}_{fit_model}",
    ...
    if manifest.fields:
        try:
            ...
            for fi in field_integration:
                _verdict(verdicts, "fields", f"energy_{fi.region}",
                         "PASS",
                         f"U={fi.total_energy_j:.3e} J, V_eff={fi.effective_mode_volume_m3 or 0:.3e} m³")
        except Exception as exc:
            _verdict(verdicts, "fields", "integration_failed",
                     "FAIL", str(exc))
    ...
    if manifest.fields and regions_path is not None:
        try:
            ...
            if loss_estimate.total_q_loss is not None:
                _verdict(verdicts, "participation", "total_q_loss",
                         "PASS",
                         f"Q_loss={loss_estimate.total_q_loss:.0f}, sum_check={loss_estimate.sum_check:.3f}")
            else:
                _verdict(verdicts, "participation", "total_q_loss",
                         "WARNING", "Could not compute total Q_loss")
        except Exception as exc:
            _verdict(verdicts, "participation", "analysis_failed",
                     "FAIL", str(exc))
    ...
    return AuditReport(
        ...
        fit_results={k: v for k, v in fit_results.items()
                     if isinstance(v, type(fit_results.get("S21", None)))},
...
def render_audit_html(report: AuditReport) -> str:
    ...
    def _badge(result: str) -> str:
        colors = {
            "PASS": "#28a745", "WARNING": "#ffc107",
            "FAIL": "#dc3545", "NOT_EVALUATED": "#6c757d",
        }
        color = colors.get(result, "#6c757d")
        return f'<span style="background:{color};color:white;padding:2px 8px;border-radius:4px;font-weight:bold">{result}</span>'

    rows = "\n".join(
        f"<tr><td>{v.section}</td><td>{v.check}</td><td>{_badge(v.result)}</td>"
        f"<td>{v.detail}</td></tr>"
...
<head>
<meta charset="utf-8">
<title>QResAudit Report — {Path(report.bundle_path).name}</title>
<style>
...
<p>Bundle: <code>{report.bundle_path}</code><br>
Audit: {report.audit_timestamp_utc.isoformat()}<br>
Schema: {report.schema_version}</p>
...
<h2>Fit Results</h2>
<pre>{json.dumps(
    {k: {sk: sv for sk, sv in v.model_dump().items()
         if sk not in ("bootstrap_confidence_95", "parameter_correlation")}
     for k, v in report.fit_results.items()},
...
def render_audit_markdown(report: AuditReport) -> str:
    ...
    for v in report.verdicts:
        lines.append(f"| {v.section} | {v.check} | {v.result} | {v.detail} |")
    return "\n".join(lines) + "\n"
...
def write_audit_output(report: AuditReport, output_dir: Path) -> None:
    ...
    (output_dir / "report.html").write_text(
        render_audit_html(report), encoding="utf-8",
    )
